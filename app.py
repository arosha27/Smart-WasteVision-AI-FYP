...
# ✅ NEW: temperature calibration saved by the improved training pipeline
CALIBRATION_PATH = os.path.join(_DIR, "weights", "calibration.json")
...
def _build_gradcam_model(cnn_model, backbone):
    ...
    try:
        ...
        # ✅ FIX 1: Direct preprocess_input call — no Lambda wrapper
        x_prep   = efficientnet_preprocess(gc_inp)
        conv_out, bb_out = backbone_dual(x_prep, training=False)

        # ✅ FIX 2: Account for CBAM — GAP replaces spatial map for Grad-CAM head
...
def load_all_artefacts():
    ...
    # ✅ FIX 1: Rebuild model architecture matching the improved training pipeline.
    # Direct preprocess_input call instead of a Lambda layer — reliably exportable.
    inp  = layers.Input(shape=(224, 224, 3), name="input_images")
    x    = efficientnet_preprocess(inp)           # no Lambda wrapper
    x    = backbone(x, training=False)

    # ✅ FIX 2: CBAM attention block must be present to match saved weights.
    # We reconstruct just enough to align with the checkpoint's layer graph.
    n_channels = backbone.output_shape[-1]        # 1536 for EfficientNetB3
    ...
    class ChannelAttention(layers.Layer):
        ...
        def call(self, inputs):
            ...
            scale = tf.reshape(tf.nn.sigmoid(avg + mx), (-1, 1, 1, tf.shape(inputs)[-1]))
            return inputs * scale
    ...
    class SpatialAttention(layers.Layer):
        ...
        def call(self, inputs):
            ...
            return inputs * self.conv(tf.concat([avg, mx], axis=-1))
    ...
    cnn_model         = models.Model(inp, out,  name="EfficientNetB3_CBAM")
    ...
    # ✅ FIX 3: Load optimal temperature for calibrated inference.
    # Falls back to T=1.0 (no calibration) if the file is absent.
    temperature = 1.0
    if os.path.exists(CALIBRATION_PATH):
        with open(CALIBRATION_PATH) as fh:
            temperature = float(json.load(fh).get("temperature", 1.0))

    gradcam_model = _build_gradcam_model(cnn_model, backbone)

    return cnn_model, feature_extractor, elm_payload, scaler, class_indices, gradcam_model, temperature
...
def preprocess_image(img: Image.Image) -> np.ndarray:
    # ✅ FIX 1: No manual rescaling — efficientnet_preprocess inside the model
    # handles pixel normalisation. Raw [0, 255] float array is correct here.
...
def _apply_temperature(probs: np.ndarray, temperature: float) -> np.ndarray:
    """
    ✅ FIX 3: Temperature scaling — post-training confidence calibration.
    Converts softmax probs → logits → scaled softmax.
    T > 1 softens overconfident predictions; T = 1 is a no-op.
    """
    if temperature == 1.0:
        return probs
...
def run_inference(img: Image.Image, model_mode: str,
                  ...
                  class_indices: dict, temperature: float = 1.0) -> dict:
    ...
    # ✅ FIX 3: Apply temperature calibration before OOD check and result reporting
    ...
    with st.sidebar:
        ...
        <div class="sb-info">
          ...
          <div class="sb-info-row"><div class="sb-info-dot"></div>
            <p class="sb-info-text">EfficientNetB3 + CBAM Attention</p></div>
          <div class="sb-info-row"><div class="sb-info-dot"></div>
            <p class="sb-info-text">Focal Loss · Label Smoothing</p></div>
          <div class="sb-info-row"><div class="sb-info-dot"></div>
            <p class="sb-info-text">Temperature-Calibrated Confidence</p></div>
          <div class="sb-info-row"><div class="sb-info-dot"></div>
            <p class="sb-info-text">7× ELM Ensemble</p></div>
          <div class="sb-info-row"><div class="sb-info-dot"></div>
            <p class="sb-info-text">36 Waste Classes · 3-Stage Hierarchy</p></div>
          <div class="sb-info-row"><div class="sb-info-dot"></div>
...
def main() -> None:
    ...
    <div class="wv-header">
      ...
      <div class="wv-hdr-right">
        <div class="wv-badge">
          ...
          EfficientNetB3 + CBAM + ELM &nbsp;·&nbsp; 36 Classes &nbsp;·&nbsp; 3 Stages
        </div>
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if missing:
        ...
        st.info("Expected layout:\n```\nweights/\n  best_accuracy.weights.h5\n"
                "  elm_ensemble.pkl\n  feature_scaler.pkl\n  class_indices.json\n"
                "  calibration.json  ← optional, enables temperature scaling\n```")
        return
    # calibration.json is optional — missing file → T=1.0 (no-op), no error shown

    try:
        cnn_model, feat_ext, elm_payload, scaler, class_indices, gradcam_model, temperature = \
            load_all_artefacts()
    ...
    st.markdown("""
    <div class="wv-footer">
      <p>WasteVision AI &nbsp;·&nbsp; EfficientNetB3 + CBAM &nbsp;·&nbsp;
         Focal Loss + Label Smoothing &nbsp;·&nbsp; Mixup Augmentation &nbsp;·&nbsp;
         Temperature Calibration &nbsp;·&nbsp; 7× ELM Ensemble &nbsp;·&nbsp; TTA &nbsp;·&nbsp; Grad-CAM</p>
    </div>
    """, unsafe_allow_html=True)
